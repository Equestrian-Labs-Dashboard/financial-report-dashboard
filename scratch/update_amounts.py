...
# 1. Remove duplicate shopifyKpis above the table
# The first one is right after variationsSection
remove_pattern = r'      <div class="section-title"><span>02 \xb7 Shopify KPI Cards</span></div>\s*<section class="kpi-grid" id="shopifyKpis"></section>\s*'
content = re.sub(remove_pattern, '', content, count=1)

# Also rename "01  Variacin" to "01  Variations (vs Previous)" and ensure english
content = re.sub(r'<div class="section-title"><span>01 [^<]+Variaci[^<]+</span></div>', r'<div class="section-title"><span>01 &middot; Variations (vs Previous)</span></div>', content)
content = re.sub(r'<div class="section-title"><span>01 [^<]+Shopify KPI Cards</span></div>', r'<div class="section-title"><span>02 &middot; Shopify KPI Cards</span></div>', content)

# 2. Modify renderVariations in JS
old_func_start = "    function renderVariations(filters) {"
old_func_end = "    function highlightSelectedPeriod(filters) {"
match = re.search(r'(    function renderVariations\(filters\) \{.*?\n)    function highlightSelectedPeriod\(filters\) \{', content, re.DOTALL)

if match:
    old_func = match.group(1)
    new_func = '''    function renderVariations(filters) {
      ...
      comparisons.forEach(comp => {
        ...
        const formatMoneyVar = (val) => (val > 0 ? "+" : "") + compactMoney(val);
