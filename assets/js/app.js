...
function renderHeader() {
  ...
  meta.version = "1.2";
  document.getElementById("versionBadge").textContent = "v2.1";
...
function renderCommercial() {
  ...
  STATE.commercial.forEach(block => {
    ...
    if (block.title && block.title.includes("Acquisition Strategy")) {
      ...
      card.appendChild(el("p", { class: "source-note" }, `CAC source: ${source}. Forecast CAC values are editable assumptions seeded from the latest actual.`));
    }
...
function applyTheme(theme) {
  ...
  localStorage.setItem("som_theme_v210", next);
...
function initThemeToggle() {
  const saved = localStorage.getItem("som_theme_v210") || "light";
  applyTheme(saved);
  const btn = document.getElementById("themeToggle");
